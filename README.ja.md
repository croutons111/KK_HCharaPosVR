...
> **β 版** — HF Patch 3.36 ＋ Meta Quest 2 で動作確認済み。最新版は [Releases](../../releases) を参照。
...
- **KK_HCharaAdjustmentEx と干渉します。** 同プラグインは自動補正だけでなく手動保存した位置も毎フレーム強制適用し続けるため、本プラグインで動かした位置が引き戻されます。自動補正の無効化だけでは**不十分**です。併用時は KK_HCharaAdjustmentEx 自体を OFF（同プラグインの `General > Enabled`）にするか、本プラグイン側を OFF にしてください。
